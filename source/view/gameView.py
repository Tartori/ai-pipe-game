...
class GameView:
    ...
    def printEnterDesiredGameSizeQuestion(self):
        print("Enter the desired fields per Line (has to be a number between 3 and 6:")
    ...
    def printGameBoard(self, gameBoard: GameBoard):
        ...
        for lineIndex, line in enumerate(gameBoard.gameBoardList):
            ...
            self.__printGameBoardLeftRightValuesFromLine(gameBoard, line, lineIndex)
            self.__printGameBoardBottomValuesFromLine(gameBoard, line, lineIndex)
            self.__printBoarderLine(gameBoard)
    ...
    def __printGameBoardTopValuesFromLine(self, gameBoard: GameBoard, line, lineIndex):
        ...
        for i in range(0, 2):
            ...
            for fieldIndex, lineField in enumerate(line):
                if(gameBoard.gameBoardList[lineIndex][fieldIndex].isTopOpen):
    ...
    def __printGameBoardLeftRightValuesFromLine(self, gameBoard: GameBoard, line, lineIndex):
        currentLine = "|"
        for fieldIndex, lineField in enumerate(line):
            if(gameBoard.gameBoardList[lineIndex][fieldIndex].isLeftOpen):
                currentGameFieldValue = "   "
            else:
                currentGameFieldValue = "XXX"
            currentLine += currentGameFieldValue + "   "
            if(gameBoard.gameBoardList[lineIndex][fieldIndex].isRightOpen):
                currentGameFieldValue = "   "
            else:
    ...
    def __printGameBoardBottomValuesFromLine(self, gameBoard: GameBoard, line, lineIndex):
        ...
        for i in range(0, 2):
            currentLine = "|"
            for fieldIndex, lineField in enumerate(line):
                if(gameBoard.gameBoardList[lineIndex][fieldIndex].isBottomOpen):
                    currentGameFieldValue = "XXX   XXX"
                else:
                    currentGameFieldValue = "XXXXXXXXX"
                currentLine += currentGameFieldValue + "|"
